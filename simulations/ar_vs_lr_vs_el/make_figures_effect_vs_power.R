...
for(i in n){
  ...
  file_path = paste0("simulations/ar_vs_lr_vs_el/results/", 
                     pdist, 
                     "_effect_vs_power_", i, "n.txt")
  
  D = read.table(file_path, header = TRUE)
  
  D$Method = ifelse(D$Method == "ar_pop_cor", 
                       "AR (pop. cor)",
                       D$Method)
  
  D$Method = ifelse(D$Method == "ar_sample_cor", 
                       "AR (sample cor)",
                       D$Method)
  
  D$Method = ifelse(D$Method == "empirical_likelihood", 
                    "EL",
                    D$Method)
...
fn = paste0("simulations/ar_vs_lr_vs_el/figures/effect_vs_power_", pdist, ".png")
